  ...
  Widget _buildMenuItems() {
    ...
    final List<Widget> items = [
      ...
      if (type == MediaType.song || type == MediaType.album) ...[
        _buildAssetMenuItem(
          context,
          icon: 'assets/icons/add-to-first.png',
          text: AppLocalizations.of(context).addToQueueNext,
          onTap: () {
            Navigator.pop(context);
            _handleAddToQueue(ref, widget.item, context, next: true);
          },
        ),
        _buildAssetMenuItem(
          context,
          icon: 'assets/icons/add-to-last.png',
          text: AppLocalizations.of(context).addToQueueLast,
          onTap: () {
            Navigator.pop(context);
            _handleAddToQueue(ref, widget.item, context, next: false);
          },
        ),
      ],
...
Future<void> _handleShare(SongMediaItem item, BuildContext context) async {
  ...
  if (item is SongDetail) {
    String artistStr = item.contributors.all
        .map((a) => a.title)
        .toSet()
        .join(', ');
  ...
  if (item is SongDetail) {
    if (!isInUpcomingQueue(audioHandler, item.id)) {
      if (next) {
        ...
        await audioHandler.addSongToQueue(item);
      }
    ...
    if (album != null && album.songs.isNotEmpty) {
      ...
      if (next) {
        final toAdd = albumSongs
            .where((s) => !isInUpcomingQueue(audioHandler, s.id))
            .toList();
        await audioHandler.addSongsNext(toAdd);
      } else {
...
Widget _buildAssetMenuItem(
  ...
  final double iconSize = isLiked ? 20.0 : 24.0;
