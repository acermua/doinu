...
class _PlaylistViewerState extends ConsumerState<PlaylistViewer> {
  ...
  Color playlistCoverColor = Colors.black;
  ...
  Future<void> _fetchPlaylist() async {
    ...
    try {
      ...
      final playlist = await sb.fetchPlaylistById(
      ...
      if (_playlist!.songCount != null &&
          _playlist!.songCount! > _playlist!.songs.length) {
        final List<SongDetail> allSongs = List.from(_playlist!.songs);

        while (allSongs.length < _playlist!.songCount!) {
          int pageToFetch = allSongs.length ~/ fetchLimit;

          final nextBatch = await sb.fetchPlaylistById(
            playlistId: widget.playlistId,
            page: pageToFetch,
            limit: fetchLimit,
          );
          ...
          final newSongs = nextBatch.songs
        ...
        _playlist = Playlist(
          ...
          language: _playlist!.language,
          songCount: _playlist!.songCount,
          explicitContent: _playlist!.explicitContent,
          songs: allSongs,
          artists: _playlist!.artists,
        );
      }
      ...
      for (int start = 0; start < totalSongs; start += batchSize) {
        ...
        final batchDetails = await sb.getSongDetails(ids: batchIds);
      ...
      final playlists = ref.read(frequentPlaylistsProvider);
      similarPlaylist =
          playlists.where((p) => p.id != widget.playlistId).toList()..shuffle();
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [playlistCoverColor, AppTheme.bg],
        ),
      ),
      child: Padding(
            ...
            builder: (context, ref, _) {
              ...
              return IconButton(
                icon: Image.asset(
                  ...
                  width: 26,
                  height: 26,
                  color: hasTimer ? AppTheme.accent : Colors.white70,
                ),
                onPressed: () {
                ...
                children: [
                  ...
                  Text(
                    status == DownloadStatus.downloading
                        ? "$downloadedCount ${AppLocalizations.of(context).of} $total ${AppLocalizations.of(context).done}"
                                        ...
                                        children: [
                                          ...
                                          if (_totalPlaylistDuration > 0)
                                            Padding(
                                              ...
                                              child: Text(
                                                '${_playlistSongDetails.length} ${AppLocalizations.of(context).songs} • ${formatDuration(_totalPlaylistDuration)}',
                                                style: TextStyle(
                                                  color: Colors.white54,
                                                  fontSize: 12,
                                                ),
      ...
      leadingActions: [
        SwipeAction(
          ...
          icon: ColorFiltered(
            ...
            child: Image.asset('assets/icons/add-to-first.png', height: 20),
          ),
          performsFirstActionWithFullSwipe: true,
          onTap: (handler) async {
            final audioHandler = await ref.read(audioHandlerProvider.future);
            await audioHandler.addSongNext(song);
            info('${song.title} ${l10n.willPlayNext}', Severity.success);
            await handler(false);
          },
        ),
      ],
                    ...
                    Text(
                      [
                        ...
                        if (song.primaryArtists.isNotEmpty)
                          song.primaryArtists
                        else if (song.contributors.primary.isNotEmpty)
