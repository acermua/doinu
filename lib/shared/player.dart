...
class PlayerColour extends _$PlayerColour {
  @override
...
@Riverpod(keepAlive: true)
Stream<String?> connectedBluetoothDevice(Ref ref) async* {
...
class _MiniPlayerState extends ConsumerState<MiniPlayer> {
  ...
  Future<void> _updatePlayerCardColour() async {
    ...
    final imageUrl = song.images.last.url;
    final dominant = await getDominantColorFromImage(imageUrl);
    if (!mounted) return;

    // final color = getDominantDarker(dominant);
...
void showOutputSwitcher(BuildContext context, WidgetRef ref) {
  final bluetoothDevice = ref
      .read(connectedBluetoothDeviceProvider)
      .asData
      ?.value;
                  ...
                  child: Column(
                    children: [
                      ...
                      ListTile(
                        ...
                        title: Text(
                          AppLocalizations.of(context).thisPhone,
                          style: TextStyle(
                            fontSize: 16,
                            color: bluetoothDevice == null
                                ? AppTheme.appGreen
                                : Colors.white38,
                            fontWeight: bluetoothDevice == null
                                ? FontWeight.w600
                                : FontWeight.w400,
                          ),
                        ),
...
class _FullPlayerScreenState extends ConsumerState<FullPlayerScreen> {
  ...
  final tabConstraintWidth = const BoxConstraints(maxWidth: 600);
  ...
  Future<void> _fetchArtistDetails() async {
    ...
    final artistId = song.contributors.all.firstOrNull?.id;
    ...
    final details = await sb.fetchArtistDetailsById(artistId: artistId);
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              song?.album ?? AppLocalizations.of(context).now,
                              style: TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w500,
                                color: Colors.white54,
  ...
  Widget _artistInfoWidget() {
    return Center(
      child: Container(
        ...
        constraints: tabConstraintWidth,
        child: Padding(
                    ...
                    child: Wrap(
                      ...
                      children: [
                        ...
                        if (_artistDetails!.followerCount != null &&
                            _artistDetails!.followerCount! > 0)
                          _buildStatItem(
                        ...
                        if (_artistDetails!.dominantLanguage.trim().isNotEmpty)
                          _buildStatItem(
                            "language",
          ...
          builder: (context, snapshot) {
            ...
            return StreamBuilder<PlayerState>(
              ...
              builder: (context, stateSnapshot) {
                ...
                // Only show loading if buffering and NOT seeking
                final isLoading =
                  ...
                  children: [
                    ...
                    _ControlButton(
                      iconWidget: Image.asset(
                        ...
                        color: isShuffle ? AppTheme.accent : Colors.white70,
                        height: 24,
                        width: 24,
                      ),
                      enabled: true,
                    ...
                    SliderTheme(
                      ...
                      child: Slider(
                        ...
                        onChangeEnd: (v) async {
                          ...
                          // Delay resetting _isDragging slightly to avoid flash of spinner
                          // while the player catches up to the seek
  ...
  Future<void> _queueTap() async {
    final audioHandler = ref
        .watch(audioHandlerProvider)
        .maybeWhen(data: (handler) => handler, orElse: () => null);

    final sourceId = audioHandler?.queueSourceId;
    final sourceNature = audioHandler?.queueNature;

    final tabIndex = ref.read(tabIndexProvider);
    debugPrint(
      '--> here the tabindex $tabIndex, sourecid $sourceId, sourenature $sourceNature',
    );

    if (sourceId == null || sourceNature == null) {
      return;
    }

    final type = switch (sourceNature) {
      'album' => PlayerNavType.album,
      'artist' => PlayerNavType.artist,
      'playlist' => PlayerNavType.playlist,
      _ => null,
    };
    ...
    ref
        ...
        .navigate(
          PlayerNavCommand(type: type, id: sourceId, tabIndex: tabIndex),
        );

    // Close full player safely
    ...
    return handlerAsync.when(
      data: (handler) {
        ...
        return queueAsync.when(
                                                ...
                                                child: GestureDetector(
                                                  ...
                                                  child: Column(
                                                    ...
                                                    children: [
                                                      ref
                                                          .watch(
                                                            connectedBluetoothDeviceProvider,
                                                          )
                                                          .when(
                                                            data: (deviceName) =>
                                                                deviceName !=
                                          ...
                                          children: [
                                            ...
                                            IconButton(
                                              ...
                                              onPressed: () async {
                                                ...
                                                details.writeln(
                                                  "${AppLocalizations.of(context).sharingFrom}\n",
                                                );
                                                details.writeln(
                                                  "${AppLocalizations.of(context).song}: ${song.title}",
                                                );
                                                String artistStr =
                                                    song.primaryArtists;
                                                String albumStr =
                                                ...
                                                if (artistStr.isNotEmpty) {
                                                  details.writeln(
                                                    "${AppLocalizations.of(context).artist}(s): $artistStr",
                                                  );
                                                }
                                                if (albumStr.isNotEmpty &&
                                                    albumStr != artistStr) {
                                                  details.writeln(
                                                    "${AppLocalizations.of(context).album}: $albumStr",
                                                  );
                                                }
                                                if (song.duration != null) {
                                                  details.writeln(
                                                    "${AppLocalizations.of(context).duration}: ${song.getHumanReadableDuration()}",
                                                  );
                                                }
                                                if (song.year != null) {
                                                  details.writeln(
                                                    "${AppLocalizations.of(context).year}: ${song.year}",
                                                  );
                                                }
          ...
          error: (_, _) => Center(
            child: Text(AppLocalizations.of(context).errorLoadingQueue),
          ),
        );
      },
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (_, _) =>
          Center(child: Text(AppLocalizations.of(context).errorLoadingHandler)),
    );
  }
}
