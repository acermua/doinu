...
class Playlist extends SongMediaItem {
  ...
  factory Playlist.fromJson(Map<String, dynamic> json) {
    return Playlist(
      ...
      images: ((json['images'] ?? json['image']) as List<dynamic>? ?? [])
          .map((e) => SourceUrl.fromJson(e as Map<String, dynamic>))
          .toList(),
      url: json['url'] ?? '',
      ...
      explicitContent:
          json['explicitContent'] ?? json['explicitcontent'] ?? false,
...
class SongDetail extends Song {
  ...
  factory SongDetail.fromJson(Map<String, dynamic> json) {
    ...
    return SongDetail(
      ...
      albumId: unescape.convert(albumId.toString()),
      primaryArtists: unescape.convert(json['primaryArtists'] ?? ''),
      singers: unescape.convert(json['singers'] ?? ''),
...
class Album extends SongMediaItem {
  ...
  factory Album.fromJson(Map<String, dynamic> json) {
    ...
    return Album(
      ...
      label: unescape.convert(json['label']?.toString() ?? ''),
      explicitContent:
          json['explicitContent'] == true || json['explicitcontent'] == true,
...
class DashboardSection {
  final String id;
  final String title;
  final String contentType; // album | playlist
  ...
  factory DashboardSection.fromJson(Map<String, dynamic> json) {
    final type = json['content_type']?.toString() ?? '';
  ...
  Map<String, dynamic> toJson() => {
    ...
    'items': contentType == 'album'
        ? albums.map((a) => Album.albumToJson(a)).toList()
        : playlists.map((p) => Playlist.playlistToJson(p)).toList(),
  };
  @override
  String toString() {
    return 'DashboardSection('
...
class ArtistDetails extends Artist {
  ...
  factory ArtistDetails.fromJson(Map<String, dynamic> json) {
    ...
    return ArtistDetails(
      ...
      dominantLanguage: json['dominantLanguage']?.toString() ?? 'English',
      ...
      language: json['dominantLanguage']?.toString() ?? 'English',
      topSongs: topSongs,
      topAlbums: topAlbums,
      singles: singles,
...
class GlobalSearch {
  ...
  factory GlobalSearch.empty() {
    return GlobalSearch(
      songs: SearchResult<Song>(results: const [], total: 0),
      albums: SearchResult<Album>(results: const [], total: 0),
      artists: SearchResult<Artist>(results: const [], total: 0),
      playlists: SearchResult<Playlist>(results: const [], total: 0),
    );
  }
