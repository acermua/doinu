...
class MyAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  Future<int?> _peekNextIndex() async {
    if (_shuffleManager.isShuffling) return _shuffleManager.getNextIndex();
    return _getNextPlayableIndex();
  }
  ...
  Future<void> addSongsNext(List<SongDetail> songs) async {
    int insertIndex = (_currentIndex + 1).clamp(0, _queue.length);
    for (final song in songs) {
      if (_queue.any((s) => s.id == song.id)) continue;
      _queue.insert(insertIndex, song);
      _shuffleManager.insertSong(insertIndex, song);
      insertIndex++;
    }
  ...
  Future<void> _playCurrent({bool skipCompletedCheck = false}) async {
    ...
    if (song.downloadUrls.isEmpty) {
      ...
      if (skipCompletedCheck) {
        final next = await _peekNextIndex();
        if (next == null) {
          await softStop();
          return;
        }
        _currentIndex = next;
        _shuffleManager.updateCurrentIndex(next);
        await _playCurrent(
          skipCompletedCheck: true,
        ); // recurse, retry cap still guards
        return;
      }
    ...
    try {
      ...
      final newMediaItem = songToMediaItem(song);
      _metadataSyncing = true;

      // Force state to loading first — signals track change to AVRCP
      playbackState.add(
        playbackState.value.copyWith(
      ...
      // Wait one frame for stack to register metadata before play state flips
